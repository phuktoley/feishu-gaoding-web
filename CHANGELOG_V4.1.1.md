...
### 2. 飞书上传ZIP失败 (配置问题，非代码bug)

**问题描述**: 上传图片到飞书时显示全部失败。

**根因分析**:
通过查看Railway部署日志发现：
- 图片上传到飞书云存储是**成功**的（获得file_token）
- 但更新记录时失败，错误码 `1254045 FieldNameNotFound`
- 原因：用户的飞书多维表格中没有名为"封面图片"的附件字段

**解决方案**:
用户需要在飞书多维表格中添加一个**附件类型**的字段，字段名设为"封面图片"（或在配置页面修改"图片字段名称"为实际存在的附件字段名）。
