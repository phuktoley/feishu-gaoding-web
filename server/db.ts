...
import { InsertUser, users, feishuConfig, InsertFeishuConfig, tasks, InsertTask } from "../drizzle/schema";
...
export async function upsertFeishuConfig(config: InsertFeishuConfig) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  const existing = await getFeishuConfig(config.userId);
  ...
  if (existing) {
    await db.update(feishuConfig)
      .set({
        ...
        imageFieldName: config.imageFieldName,
      })
      .where(eq(feishuConfig.userId, config.userId));
  } else {
    await db.insert(feishuConfig).values(config);
  }
}
